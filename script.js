const settings = { //General configuration settings for the program (modify this to easily modify the program)
  ...
  starting: {
    possibleColors: ["orange", "lightgreen"], //Possible colors for the fish to start
    min:8,
    max:14,
  },
...
var gameObjects = { //Stores the classes used to create objects in the game
  tank: function(xpos, ypos, width, height) { //The class that manages the bodies (tanks) of water
    ...
    //Keep track of all the tanks in the "game", for detecting mouse clicks & mouse hober
    ...
    this.addFish = function(fish) {
      ...
      fish.rotation = (fish.xpos > this.xpos + (this.width / 2)) * 180 //Here, the fish will go right if it's on the right side of the tank, or left if it's on the left side of the tank
      ...
      //Keep track of the quantity of a certain color of fish
      if(this.fishScoreboard[fish.color] === undefined) this.fishScoreboard[fish.color] = 0; //Add to the fish scoreboard
      this.fishScoreboard[fish.color]++;
    }
    this.removeFish = function(fish) {
      ...
      if(this.fishScoreboard[fish.color] !== undefined) {
        this.fishScoreboard[fish.color]--;
        if(settings.features.removeZeroFish && this.fishScoreboard[fish.color] <= 0) delete this.fishScoreboard[fish.color];
      }
  ...
  fish: function(color) {
    ...
    //Set default configurations
    this.rotation = 0;
    this.wander = false; //This denotes if the fish should try to "wander" idley, e.g. swim back and forth within its tank
    this.canPickUp = true;
...
function setup() {
  ...
  if(settings.features.addFish) buildAddFish();
...
function renderProbabilityInterface(pond, x, y) {
  ...
  if(pond.fish.length > 0) { //Optimization: Only render probabilities if there are fish at all
    ...
    for(var color in pond.fishScoreboard) { //Iterate through all available colors
      textSize(14);
      fill(color);
      ...
      text(pond.fishScoreboard[color], x + (settings.interface.tableSpacing * 2), rowPositionY);

      //Render collumn 2 - the fractional probability
      text(pond.fishScoreboard[color] + "/" + pond.fish.length, x + (settings.interface.tableSpacing * 3), rowPositionY);

      //Render collumn 3 - the percent chance
      var percent = ((pond.fishScoreboard[color] / pond.fish.length) * 100)
...
function buildAddFish() { //This function builds the "Add Fish" button and its associated color buttons. Note that this only needs to be run once, since buttons are stored directly in HTML DOM.
...
function populateStarting(pond) {
  for(var i = 0; i <= rand(settings.starting.min, settings.starting.max + 1); i++) {
    var color = settings.starting.possibleColors[rand(0, settings.starting.possibleColors.length)]; //Choose a random color from the list of possible colors
